---
title: "Appendix S1: BRMS models' validation"
output: html_document
---

This document describes the output and validation of the statistical models presented in the main text.
...
```{r import model 1}
model.SA<-readRDS("../04_bayesian_analysis/models/SA_quantile.rds")
```


## 1.1 Model and convergence diagnostics 

**Table S1.1: Summary table of the BRMS model results.** The table summarizes the posterior distributions of the fixed effects and interaction terms. Reported values include the posterior means, standard errors, 95% credible intervals (CI), and effective sample sizes (Bulk ESS and Tail ESS) for each parameter.
...
**Figure S1.1: Trace plots allow visually assessing chain convergence.**
...
**Figure S1.2: The density plot compares the real data (`y`, dark blue line) to the fitted draws of the models (`y_rep`, light blue lines).**

#### 1.2.1.2 Leave-One-Out Cross-Validation

```{r warning=FALSE, message=FALSE}
pp_check(model.SA, type="loo_pit")+ 
  theme_bw() 
```

**Figure S1.3: The LOO-PIT (Leave-One-Out Probability Integral Transform) calibration plot compares the empirical cumulative distribution of the LOO-PIT values (solid blue line) against the uniform distribution (dashed diagonal line).**
...
**Figure S1.4: We use the highest density interval (HDI) for the main effects in the model to characterize uncertainty of our posterior distributions.** 


## 1.3 The Hierarchical Effect of Species Richness on Spatial Asynchrony

**Table S1.2: Summary table of the hierarchical effects of planted species richness on spatial asynchrony.**
...
**Figure S1.5. Posterior distributions of the random effects of planted species richness levels on gamma stability, comparing differences between species richness levels.** Black points represent posterior medians, and horizontal black lines indicate the 66% and 95% credible intervals. The dashed vertical line at x = 0 serves as a reference for no difference. Deviations from zero indicate the magnitude and direction of the differences in spatial asynchrony between species richness levels.


# 2. Effects of Spatial Asynchony and Spatial Extent on Gamma Stability

```{r import model 2}
model.GS<-readRDS("../04_bayesian_analysis/models/GS_quantile.rds")
```

## 2.1 Model and convergence diagnostics 

**Table S1.3: Summary table of the BRMS model results.** The table summarizes the posterior distributions of the fixed effects and interaction terms. Reported values include the posterior means, standard errors, 95% credible intervals (CI), and effective sample sizes (Bulk ESS and Tail ESS) for each parameter.
...
**Figure S1.6: Trace plots allow visually assessing chain convergence.**
...
Figure S1.7: The density plot compares the real data (`y`, dark blue line) to the fitted draws of the models (`y_rep`, light blue lines).**

#### 2.2.1.2 Leave-One-Out Cross-Validation

```{r warning=FALSE, message=FALSE}
pp_check(model.GS, type="loo_pit")+ 
  theme_bw() 
```

**Figure S1.8: The LOO-PIT (Leave-One-Out Probability Integral Transform) calibration plot compares the empirical cumulative distribution of the LOO-PIT values (solid blue line) against the uniform distribution (dashed diagonal line).**
...
**Figure S1.9: We use the highest density interval (HDI) for the main effects in the model to characterize uncertainty of our posterior distributions.** 


## 2.3 The Hierarchical Effect of Species Richness on Spatial Asynchrony

**Table S1.4: Summary table of the hierarchical effects of planted species richness on gamma stability.**
...
**Figure S1.10. Posterior distributions of the random effects of planted species richness levels on gamma stability, comparing differences between species richness levels.** Black points represent posterior medians, and horizontal black lines indicate the 66% and 95% credible intervals. The dashed vertical line at x = 0 serves as a reference for no difference. Deviations from zero indicate the magnitude and direction of the differences in gamma stability between species richness levels.
