---
title: "Plot Gamma Stability"
output: html_document
---

Running this document produces Figure 3 of the main text.
...
filtered_data <- filtered_data %>%
  select(Spatial_AS, Gamma_stab, Area, div) 
```

```{r read in the model}
GS_quantile <- readRDS("./models/GS_quantile.rds")
```
...
GS_prediction_curves <- filtered_data %>% 
  add_epred_draws(GS_quantile, ndraws = 5000) %>%  #specify number of draws only during initial stages of plotting
  ggplot(aes(x = Spatial_AS, y = Gamma_stab, group = Area, color = Area)) + # group by Area to avoid messy lines
...
# Print the plot
print(GS_prediction_curves)

# Save the plot using ggsave()
ggsave("./figures/Fig.3A.png", plot = GS_prediction_curves, width = 8, height = 6, dpi = 600)
...
GS_prediction_curves_beta2_area.facet <- filtered_data %>% 
  ...
  coord_cartesian(xlim = c(1, 1.4),
                  ylim = c(1,3.5)) + 
  # Mofify axes's appearance
  scale_x_continuous(breaks = c(1,1.2,1.4))+ # reduce number of x axis ticks
  theme(axis.title = element_text(size = 20, colour = "black"), 
        axis.line.x = element_line(colour = "black"),
        axis.line.y = element_line(colour = "black"),
...
# Print the plot
print(GS_prediction_curves_beta2_area.facet)

# Save the plot using ggsave()
ggsave("./figures/Fig.3B.png", plot = GS_prediction_curves_beta2_area.facet, width = 8, height = 6, dpi = 600)
...
theme_set(theme_tidybayes() + panel_border())
filtered_data %>% 
  add_epred_draws(GS_quantile, ndraws = 5000) %>%  # Add predicted draws from model
